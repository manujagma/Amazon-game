...
def run_classic_mode(screen, clock):
    ...
    # Big mother ship properties
    big_mother_ship = None
    big_mother_ship_health = 15
    big_mother_ship_speed = 1.5
    big_mother_ship_bullets = []
    big_mother_ship_split_bullets = []
    big_mother_ship_shoot_cooldown = 0
    big_mother_ship_shoot_delay = 90  # Frames between shots
    ...
    def draw_big_mother_ship(x, y, health):
        # Draw big mother ship body (hexagon shape)
        pygame.draw.polygon(screen, CYAN, [
            (x + BIG_MOTHER_SHIP_SIZE // 4, y),  # Top left
            (x + BIG_MOTHER_SHIP_SIZE * 3 // 4, y),  # Top right
            (x + BIG_MOTHER_SHIP_SIZE, y + BIG_MOTHER_SHIP_SIZE // 2),  # Right
            (x + BIG_MOTHER_SHIP_SIZE * 3 // 4, y + BIG_MOTHER_SHIP_SIZE),  # Bottom right
            (x + BIG_MOTHER_SHIP_SIZE // 4, y + BIG_MOTHER_SHIP_SIZE),  # Bottom left
            (x, y + BIG_MOTHER_SHIP_SIZE // 2)  # Left
        ])
        # Draw center core
        pygame.draw.circle(screen, RED, (x + BIG_MOTHER_SHIP_SIZE // 2, y + BIG_MOTHER_SHIP_SIZE // 2), BIG_MOTHER_SHIP_SIZE // 4)
        # Draw energy rings
        pygame.draw.circle(screen, YELLOW, (x + BIG_MOTHER_SHIP_SIZE // 2, y + BIG_MOTHER_SHIP_SIZE // 2), BIG_MOTHER_SHIP_SIZE // 3, 3)
        pygame.draw.circle(screen, YELLOW, (x + BIG_MOTHER_SHIP_SIZE // 2, y + BIG_MOTHER_SHIP_SIZE // 2), BIG_MOTHER_SHIP_SIZE // 5, 2)
        # Draw main cannon
        pygame.draw.rect(screen, CYAN, (x + BIG_MOTHER_SHIP_SIZE // 2 - 10, y + BIG_MOTHER_SHIP_SIZE, 20, 15))
        # Draw health bar
        health_width = (BIG_MOTHER_SHIP_SIZE * health) // 15
        pygame.draw.rect(screen, RED, (x, y - 15, BIG_MOTHER_SHIP_SIZE, 10))
        pygame.draw.rect(screen, GREEN, (x, y - 15, health_width, 10))

    def draw_bullet(x, y, enemy=False, big_bullet=False):
        if big_bullet:
            pygame.draw.circle(screen, CYAN, (x + BULLET_SIZE, y + BULLET_SIZE), BULLET_SIZE * 2)
        elif enemy:
    ...
    def show_victory():
        # Semi-transparent overlay
        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 128, 128))  # Blue with alpha
        screen.blit(overlay, (0, 0))
        
        font = pygame.font.SysFont(None, 72)
        victory_text = font.render("VICTORY!", True, YELLOW)
        screen.blit(victory_text, (SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT // 2 - 86))
        
        font = pygame.font.SysFont(None, 36)
        congrats_text = font.render("You saved the galaxy!", True, WHITE)
        screen.blit(congrats_text, (SCREEN_WIDTH // 2 - 140, SCREEN_HEIGHT // 2 - 40))
        
        menu_text = font.render("Press M for Main Menu", True, WHITE)
        screen.blit(menu_text, (SCREEN_WIDTH // 2 - 140, SCREEN_HEIGHT // 2 + 50))

    def reset_game():
        nonlocal player_x, player_y, player_bullets, enemies, score, game_over, game_won
        ...
        nonlocal big_mother_ship, big_mother_ship_bullets, big_mother_ship_split_bullets, big_mother_ship_health
    ...
    while not return_to_menu:
        ...
        if not game_over and not game_won:
            ...
            # Spawn enemies only if big mother ship is not present
            ...
            # Big mother ship movement and shooting
            if big_mother_ship:
                # Move big mother ship side to side
                big_mother_ship[0] += big_mother_ship_speed
                if big_mother_ship[0] <= 0 or big_mother_ship[0] + BIG_MOTHER_SHIP_SIZE >= SCREEN_WIDTH:
                    big_mother_ship_speed = -big_mother_ship_speed
                
                # Big mother ship shooting
                big_mother_ship_shoot_cooldown -= 1
                if big_mother_ship_shoot_cooldown <= 0:
                    # Create a single bullet at the bottom of the big mother ship
                    bullet_x = big_mother_ship[0] + BIG_MOTHER_SHIP_SIZE // 2 - BULLET_SIZE
                    bullet_y = big_mother_ship[1] + BIG_MOTHER_SHIP_SIZE
                    # Store bullet with its position and a flag to indicate it hasn't split yet
                    big_mother_ship_bullets.append([bullet_x, bullet_y, False])
                    big_mother_ship_shoot_cooldown = big_mother_ship_shoot_delay
            
            # Move big mother ship bullets and check for splitting
            for bullet in big_mother_ship_bullets[:]:
                bullet[1] += bullet_speed // 2
                
                # Check if bullet has reached center of screen and hasn't split yet
                if bullet[1] >= SCREEN_CENTER_Y and not bullet[2]:
                    # Mark bullet as split
                    bullet[2] = True
                    
                    # Create 4 split bullets going in different directions
                    # Up-left
                    big_mother_ship_split_bullets.append([bullet[0], bullet[1], -bullet_speed // 3, -bullet_speed // 3])
                    # Up-right
                    big_mother_ship_split_bullets.append([bullet[0], bullet[1], bullet_speed // 3, -bullet_speed // 3])
                    # Down-left
                    big_mother_ship_split_bullets.append([bullet[0], bullet[1], -bullet_speed // 3, bullet_speed // 3])
                ...
                if bullet[1] > SCREEN_HEIGHT:
                    big_mother_ship_bullets.remove(bullet)
            
            # Move big mother ship split bullets
            for bullet in big_mother_ship_split_bullets[:]:
                # Move in the specified direction (x and y have separate velocities)
                bullet[0] += bullet[2]  # x velocity
                bullet[1] += bullet[3]  # y velocity
                
                # Remove bullets that go off screen
                if (bullet[0] < 0 or bullet[0] > SCREEN_WIDTH or 
                    bullet[1] < 0 or bullet[1] > SCREEN_HEIGHT):
                    big_mother_ship_split_bullets.remove(bullet)
            ...
            for bullet in player_bullets[:]:
                ...
                # Collision detection: bullets hitting big mother ship
                if big_mother_ship and (bullet[0] < big_mother_ship[0] + BIG_MOTHER_SHIP_SIZE and
                    bullet[0] + BULLET_SIZE > big_mother_ship[0] and
                    bullet[1] < big_mother_ship[1] + BIG_MOTHER_SHIP_SIZE and
                    bullet[1] + BULLET_SIZE * 2 > big_mother_ship[1]):
                    
                    if bullet in player_bullets:
                        player_bullets.remove(bullet)
                    big_mother_ship_health -= 1
                    
                    # Big mother ship destroyed
                    if big_mother_ship_health <= 0:
                        score += 250
                        big_mother_ship = None
                        big_mother_ship_health = 15
                        big_mother_ship_bullets = []
            ...
            # Collision detection: player hitting big mother ship bullets
            for bullet in big_mother_ship_bullets[:]:
                if (player_x < bullet[0] + BULLET_SIZE * 2 and
                    player_x + PLAYER_SIZE > bullet[0] and
                    player_y < bullet[1] + BULLET_SIZE * 2 and
                    player_y + PLAYER_SIZE > bullet[1]):
                    game_over = True
            
            # Collision detection: player hitting big mother ship split bullets
            for bullet in big_mother_ship_split_bullets[:]:
                if (player_x < bullet[0] + BULLET_SIZE and
                    player_x + PLAYER_SIZE > bullet[0] and
                    player_y < bullet[1] + BULLET_SIZE * 2 and
        ...
        # Draw big mother ship bullets
        for bullet in big_mother_ship_bullets:
            draw_bullet(bullet[0], bullet[1], enemy=True, big_bullet=True)
        
        # Draw big mother ship split bullets
        for bullet in big_mother_ship_split_bullets:
            draw_bullet(bullet[0], bullet[1], enemy=True)
        ...
        # Draw big mother ship if it exists
        if big_mother_ship:
            draw_big_mother_ship(big_mother_ship[0], big_mother_ship[1], big_mother_ship_health)
